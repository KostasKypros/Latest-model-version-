# Pheno in IC wiht old format 

### Phenotypes in IC Cleaner output

### Phenotypes in IC 1 output file 
### phenotypes in intercrops code
...
module SpeciesParams
    export get_species_params, species_list, get_phenotype_params, phenotype_list
    const species_list = [:cereal1, :cereal2]
    # set phenotype list 
    const phenotype_list = [:reference, :topsoil, :deep]
    ...
    function get_species_params(spec::Symbol)
        ...
        if spec === :cereal1
            ...
            return tp(
                ...
                IBD = 0.003, root_meristem_count = 1, base_tilt = 53.3, base_tilt_jitter = 3.4,
                seminal_tilt = 0.0, seminal_tilt_jitter = 10.0, ER = 0.056, ER_accumulator = 0.0,
                ER_juvenile_lag_days = 0, nodal_count = 0, max_nodals = 6
            )
        end
    end   # ← end of get_species_params

    # Root-phenotype variants of Species 2 (:cereal2).
    # Overrides ONLY base_tilt, max_nodals, IBD. Everything else = Species 2.
    function get_phenotype_params(pheno::Symbol)
        @assert pheno in phenotype_list "unknown phenotype: $pheno"
        tp = get_species_params(:cereal2)   # start from Species 2 exactly
        if pheno === :reference
            tp.base_tilt  = 53.3
            tp.max_nodals = 6
            tp.IBD        = 0.003
        elseif pheno === :topsoil
            tp.base_tilt  = 37.3
            tp.max_nodals = 8
            tp.IBD        = 0.002
        else # :deep
...
end   # ← end of module SpeciesParams
...
function create_tree(origin, orientation, species::Symbol; phenotype::Union{Symbol,Nothing} = nothing)
        vars = phenotype === nothing ?
           SpeciesParams.get_species_params(species) :
           SpeciesParams.get_phenotype_params(phenotype)
...
function write_run_metadata(path::String;
    ...
    df = DataFrame(
        ...
        RUE_sp2            = rue_sp2,
        EN_coeff           = EN_coeff,
        pheno1             = String(pheno1),
        pheno2             = String(pheno2),
        pheno1_base_tilt   = Main.SpeciesParams.get_phenotype_params(pheno1).base_tilt,
        pheno1_max_nodals  = Main.SpeciesParams.get_phenotype_params(pheno1).max_nodals,
        pheno1_IBD         = Main.SpeciesParams.get_phenotype_params(pheno1).IBD,
        pheno2_base_tilt   = Main.SpeciesParams.get_phenotype_params(pheno2).base_tilt,
        pheno2_max_nodals  = Main.SpeciesParams.get_phenotype_params(pheno2).max_nodals,
        pheno2_IBD         = Main.SpeciesParams.get_phenotype_params(pheno2).IBD,
...
# collector
function root_voxel_occupancy(forest, soil, plant_ids, DOY::Int; plant_row=nothing, plant_pheno=nothing)
    ...
    # per-(voxel, plant, seg_type) long table
       pp = DataFrame(DOY=Int[], cell=Int[], i=Int[], j=Int[], k=Int[], x=Float64[], y=Float64[], z=Float64[],
                   plant_id=Int[], row=Int[], phenotype=Symbol[],
                   seg_type=Symbol[], root_length=Float64[], root_biomass=Float64[])
    for ((si, pid, stype), (L, B)) in acc
        (i,j,k,x,y,z) = cellmeta[si]
        rw = plant_row   === nothing ? 0        : get(plant_row, pid, 0)
        ph = plant_pheno === nothing ? :unknown : get(plant_pheno, pid, :unknown)
        push!(pp, (DOY, si, Int(i), Int(j), Int(k), x, y, z, pid, rw, ph, stype, L, B))
    end
...
# Per-plant N uptake (actual vs potential) aggregated by soil depth layer (k).
function n_uptake_by_depth(forest, soil, plant_ids, DOY::Int; plant_row=nothing, plant_pheno=nothing)
    soil_cells = get_soilcells(soil)
    # depth of each z → nearest cell k; build z→k lookup from cells
    # acc[(pid,k)] = (potNup, actNup)
    acc = Dict{Tuple{Int,Int}, NTuple{2,Float64}}()
    # map a segment's z to a k by matching the closest soil cell k centre
    kz = sort(unique([(Int(round(sc.k)), sc.z) for sc in soil_cells]); by = x->x[2])
    nearest_k(z) = isempty(kz) ? 0 : kz[argmin([abs(z - c[2]) for c in kz])][1]

    for (tree, pid) in zip(forest, plant_ids)
        for rs in get_Rsegments(tree)
            k = nearest_k(rs.z)
            p, a = get(acc, (pid,k), (0.0,0.0)); acc[(pid,k)] = (p + rs.potNup, a + rs.actNup)
        end
        for rs in get_RsegmentsLateral(tree)
            k = nearest_k(rs.z)
            p, a = get(acc, (pid,k), (0.0,0.0)); acc[(pid,k)] = (p + rs.potNup, a + rs.actNup)
    ...
    df = DataFrame(DOY=Int[], plant_id=Int[], row=Int[], phenotype=Symbol[],
                   k=Int[], potNup=Float64[], actNup=Float64[])
    for ((pid,k),(p,a)) in acc
        rw = plant_row   === nothing ? 0        : get(plant_row, pid, 0)
        ph = plant_pheno === nothing ? :unknown : get(plant_pheno, pid, :unknown)
        push!(df, (DOY, pid, rw, ph, k, p, a))
    end
    return df
end
...
#
function soil_N_per_voxel(soil, DOY::Int)
    df = DataFrame(DOY=Int[], cell=Int[], i=Int[], j=Int[], k=Int[],
                   x=Float64[], y=Float64[], z=Float64[], soil_N=Float64[])
    for (si, sc) in enumerate(get_soilcells(soil))
        push!(df, (DOY, si, Int(sc.i), Int(sc.j), Int(sc.k), sc.x, sc.y, sc.z, sc.N))
    end
    return df
end
...
#
    pp_all = DataFrame[]
    pv_all = DataFrame[]
    nk_all = DataFrame[]   # N uptake by depth
    fn_all = DataFrame[]   # fn/CN/CNcrit per plant
    sn_all = DataFrame[]   # soil N per voxel
...
function run_full_model(; n_days::Int = 80,
                          start_DOY::Int = 180,
                          soil_N_init::Float64 = 30000.0,
                          label::String = "Full model",
                          write_split::Bool = false,
                          ...
                          pheno1::Symbol = :reference,
                          pheno2::Symbol = :reference,
                          collect_extended::Bool = true)

   # Random.seed!(rng_seed)

    # positions: (coordinates, species, block, plant_id, phenotype)
    positions = Tuple{Vec{Float64}, Symbol, Int, Int, Symbol}[]
    plant_id = 0
    y_cursor = 0.0
    ...
    for row in 1:n_rows_sp1
        ...
        for col in 1:n_plants_row_sp1
            ...
            push!(positions, (Vec(x, y, 0.0), sp1, 1, plant_id, pheno1))
        end
    end
    ...
    for row in 1:n_rows_sp2
        ...
        for col in 1:n_plants_row_sp2
            ...
            push!(positions, (Vec(x, y, 0.0), sp2, 2, plant_id, pheno2))
        end
    end
    ...
    forest_local = [create_tree(pos, rand() * 360.0, sp; phenotype = ph)
                    for (pos, sp, _, _, ph) in positions]
    ...
    n_k = round(Int, 0.5 / sv)

    voxel_volume_L = sv^3 * 1000.0
    ...
    plant_row = Dict(p[4] => p[3] for p in positions)
    plant_pheno = Dict(p[4] => p[5] for p in positions)
    ...
    pos_df = DataFrame(
        ...
        block     = [p[3] for p in positions],
        plant_id  = plant_ids,
        phenotype = [p[5] for p in positions],
    )

    for i in 1:n_days
        DOY = start_DOY + i - 1
        daily_step!(forest_local, soil_local, ss_local, DOY,
                    x_min, x_max, y_min, y_max, sl, sw)
        ...
        if collect_extended
            push!(nk_all, n_uptake_by_depth(forest_local, soil_local, plant_ids, DOY))
            push!(sn_all, soil_N_per_voxel(soil_local, DOY))

            for (p, tree) in enumerate(forest_local)
                pid = plant_ids[p]
                fn_v, CN_v, CNcrit_v, _ = compute_fn(tree)
                push!(fn_all, DataFrame(
                    DOY = DOY,
                    plant_id = pid,
                    block = get(plant_row, pid, 0),
                    phenotype = get(plant_pheno, pid, :unknown),
                    fn = fn_v,
                    CN = CN_v,
                    CNcrit = CNcrit_v
                ))
    ...
    if collect_extended
        ...
        if !isempty(pv_full) && !isempty(sn_full)
            sn_key = select(sn_full, [:DOY, :cell, :soil_N])
            pv_full = leftjoin(pv_full, sn_key, on = [:DOY, :cell])
        end
    else
        nk_full = DataFrame()
        fn_full = DataFrame()
    ...
    if write_split
        ...
        if collect_extended
            CSV.write("$(label)_n_uptake_by_depth.csv", nk_full)
            CSV.write("$(label)_fn_components.csv", fn_full)
        end
        write_run_metadata(
    ...
    # experiment stacking needs a one-row metadata DataFrame
    meta_df = DataFrame(
        label = label,
        sp1 = sp1,
        sp2 = sp2,
    ...
    return (;
        diags, forest_local, soil_local, ss_local, soil_mesh_local,
        positions, pos_df, plant_ids, plant_row, plant_pheno,
        pp_full, pv_full, nk_full, fn_full, sn_full, meta_df,
        sv, soil_N_init, label, pheno1, pheno2
    )
end
...
## full run
# ═══════════════════════════════════════════════════════════════════
# SECTION: RUN SIMULATION  → Y-STYLE COMBINED OUTPUTS
# ═══════════════════════════════════════════════════════════════════
function run_experiment_combined(; n_days::Int = 35, start_DOY::Int = 180,
                                   sv::Float64 = 0.04) #, rng_seed::Int = 123456789)

    fixed = (
        sp1 = :cereal2, sp2 = :cereal2,
        n_plants_row_sp1 = 12, n_plants_row_sp2 = 12,
        n_rows_sp1 = 1,        n_rows_sp2 = 1,
        plant_spacing_sp1 = 0.02, plant_spacing_sp2 = 0.02,
        row_spacing_sp1 = 0.12,    row_spacing_sp2 = 0.12,
        inter_row_spacing = 0.12,
    )

    configs = [
        (:reference, :reference), (:topsoil, :topsoil), (:deep, :deep),
        (:reference, :topsoil),   (:reference, :deep),  (:deep, :topsoil),
    ]
    N_levels = [30000.0, 15000.0]

    master_df      = DataFrame()
    positions_all  = DataFrame()
    rootplant_all  = DataFrame()
    rootvoxel_all  = DataFrame()
    nuptake_all    = DataFrame()
    meta_all       = DataFrame()
    forest_all     = Any[]
    ss_all         = Any[]
    sm_all         = Any[]
    labels_all     = String[]
    all_diags = Vector{DiagnosticTimeSeries}[]

    for (p1, p2) in configs, N in N_levels
        label = "$(p1)_$(p2)_N$(Int(N))"
        treat = (p1 == p2) ? :mono : :intercrop
        println("\n", "="^70); println("  RUN  $label"); println("="^70)

        r = run_full_model(; n_days=n_days, start_DOY=start_DOY, sv=sv,
                             soil_N_init=N, label=label,
                             write_split=false, pheno1=p1, pheno2=p2, fixed...)

    run_diags = DiagnosticTimeSeries[]
        for (pidx, diag) in enumerate(r.diags)
            pid = r.plant_ids[pidx]
            df_p = diag_to_dataframe(diag)
            df_p[!, :label]     .= label
            df_p[!, :N_level]   .= Int(N)
            df_p[!, :treatment] .= treat
            df_p[!, :pheno1]    .= p1
            df_p[!, :pheno2]    .= p2
            df_p[!, :phenotype] .= get(r.plant_pheno, pid, :unknown)
            df_p[!, :row]       .= get(r.plant_row,   pid, 0)
            df_p[!, :plant_id]  .= pid
            append!(master_df, df_p; cols = :union)
            push!(run_diags, diag)
        end
        push!(all_diags, run_diags)
    ...
    df
        ...
        append!(positions_all, _tag!(copy(r.pos_df)); cols=:union)
        append!(rootplant_all, _tag!(copy(r.pp_full));  cols=:union)
        append!(rootvoxel_all, _tag!(copy(r.pv_full));  cols=:union)
        append!(nuptake_all,   _tag!(copy(r.nk_full));  cols=:union)
        append!(meta_all,      _tag!(copy(r.meta_df));  cols=:union)

        push!(forest_all, r.forest_local)
        push!(ss_all,     r.ss_local)
        push!(sm_all,     r.soil_mesh_local)
        push!(labels_all, label)
    end

    diag_rep = mean_diag(vcat(all_diags...))

    return (;
        master_df, positions_all, rootplant_all, rootvoxel_all,
        nuptake_all, meta_all,
        forest_all, ss_all, sm_all, labels_all,
        all_diags, diag_rep,
        sv, n_days, start_DOY
    )
end

# stop here and run botom run 


# #run_experiment_combined(; n_days = 35, start_DOY = 180, sv = 0.04)
# using DataFrames, CSV
# import GLMakie

# result = run_experiment_combined(; n_days=35, start_DOY=180, sv=0.04) #### ###

# # run complete


# ## output per old model format file

# function render_by_label(result, label::String; savefig = true, cam = nothing)
#     k = findfirst(==(label), result.labels_all)
#     isnothing(k) && error("label '$label' not found. Available: $(result.labels_all)")

#     fig = render_forest(result.forest_all[k], result.ss_all[k], result.sm_all[k])

#     if savefig
#         GLMakie.save("scene_$(label).png", fig)
#     end

#     if !isnothing(cam)
#         scene = fig.scene
#         c = GLMakie.cameracontrols(scene)
#         GLMakie.update_cam!(scene, cam.eye, cam.lookat, cam.up)
#         GLMakie.update!(scene)
#         if savefig
#             GLMakie.save("scene_$(label)_fixedcam.png", fig)
#         end
#     end
...
# function labels_for_N(result, N::Int)
#     filter(l -> occursin("N$(N)", l), result.labels_all)
# end

# function render_by_N(result, N::Int, which::Int = 1; kwargs...)
#     labels = labels_for_N(result, N)
#     isempty(labels) && error("no run found for N=$(N)")
#     render_by_label(result, labels[which]; kwargs...)
# end

# ## example use 
# # one specific treatment
# render_by_label(result, "reference_topsoil_N30000")

# # 

# #the output storing worked 
# ## output storing
...
# positions_all = result.positions_all
# rootplant_all = result.rootplant_all
# rootvoxel_all = result.rootvoxel_all
# nuptake_all   = result.nuptake_all
# meta_all      = result.meta_all
# forest_all    = result.forest_all
# ss_all        = result.ss_all
# sm_all        = result.sm_all
# labels_all    = result.labels_all
# all_diags     = result.all_diags
# diag_rep      = result.diag_rep
# sv            = result.sv

# # ── CSVs ──
# CSV.write("all_plants.csv",            master_df)
# CSV.write("plant_positions_all.csv",   positions_all)
# CSV.write("root_per_plant_all.csv",    rootplant_all)
# CSV.write("root_per_voxel_all.csv",    rootvoxel_all)
# CSV.write("n_uptake_by_depth_all.csv", nuptake_all)
# CSV.write("run_metadata_all.csv",      meta_all)


# this run properly


## alterantive full run 

# ═══════════════════════════════════════════════════════════════════
# FINAL PHENOTYPE × N EXPERIMENT WORKFLOW
# ═══════════════════════════════════════════════════════════════════

using Random, DataFrames, CSV
import GLMakie

# ── 1. run the experiment (this is the expensive part) ──
result = run_experiment_combined(; n_days=35, start_DOY=180, sv=0.04)

# ── 2. save combined CSVs ──
CSV.write("all_plants.csv",            result.master_df)
CSV.write("plant_positions_all.csv",   result.positions_all)
CSV.write("root_per_plant_all.csv",    result.rootplant_all)
CSV.write("root_per_voxel_all.csv",    result.rootvoxel_all)
CSV.write("n_uptake_by_depth_all.csv", result.nuptake_all)
CSV.write("run_metadata_all.csv",      result.meta_all)
println("Saved combined CSVs.")

# ── 3. save global mean diagnostic plots ──
fig_mean = plot_diagnostics(result.diag_rep;
                            title_prefix = "sv=$(result.sv) m (global mean)")
savefig(fig_mean, "diag_sv$(result.sv)_mean.png")
display(fig_mean)

fig_budget = plot_carbon_budget(result.diag_rep;
                                title_prefix = "sv=$(result.sv) m (global mean)")
savefig(fig_budget, "carbon_budget_sv$(result.sv)_mean.png")
display(fig_budget)

# ── 4. scene render helpers ──
function render_by_label(result, label::String;
                         savefig::Bool = true, cam = nothing)
    k = findfirst(==(label), result.labels_all)
    isnothing(k) && error("label '$label' not found. Available: $(result.labels_all)")
    ...
    if !isnothing(cam)
        ...
        cc = GLMakie.cam3d!(lscene.scene)
        cc.eyeposition[]         = cam.eye
        cc.lookat[]              = cam.lookat
        cc.upvector[]            = cam.up
        cc.update_restrictions[] = false
        GLMakie.update!(lscene.scene)
...
# ── 5A. TEST ONE LABEL FIRST ──
render_by_label(result, "deep_deep_N15000")  
...
# test fixed angle 
function render_by_label(result, label::String; savefig::Bool = true, cam = nothing)
    k = findfirst(==(label), result.labels_all)
    isnothing(k) && error("label '$label' not found. Available: $(result.labels_all)")
    ...
    if !isnothing(cam)
        ...
        cc.lookat[]      = cam.lookat
        cc.upvector[]    = cam.up
        GLMakie.update_cam!(lscene.scene, cc)
    end

    if savefig
        suffix = isnothing(cam) ? "" : "_fixedcam"
        GLMakie.save("scene_$(label)$(suffix).png", fig)
    end
...
render_by_label(result, "deep_deep_N15000"; cam = cam)

## 3rd angle
#
cam = (
    eye    = GLMakie.Vec3f(1.88422, 0.09099, 0.10011),
    lookat = GLMakie.Vec3f(0.06517, 0.11461, -0.09738),
    up     = GLMakie.Vec3f(-0.10792, 0.00140, 0.99416)
)
...
function render_by_label(result, label::String; savefig::Bool = true, cam = nothing)
    ...
    if savefig
        suffix = isnothing(cam) ? "" : "_fixedcam2"
        GLMakie.save("scene_$(label)$(suffix).png", fig)
    end
...
okay this worked but it saved only the deep_deep_N15000 one not all of them
...
function render_by_label(result, label::String;
    ...
    if savefig
        name = isnothing(cam) ? "scene_$(label).png" : "scene_$(label)_$(suffix).png"
        GLMakie.save(name, fig)
    end
...
# TEST ONE LABEL
# ═══════════════════════════════════════════════════════════════════
lbl = "deep_deep_N15000"
render_by_label(result, lbl)                                   # default
render_by_label(result, lbl; cam=cam_fixed1, suffix="fixed1")  # fixed angle 1
render_by_label(result, lbl; cam=cam_fixed2, suffix="fixed2")  # fixed angle 2

# ═══════════════════════════════════════════════════════════════════
# BATCH RENDER ALL LABELS / ALL CAMERA ANGLES
# ═══════════════════════════════════════════════════════════════════
camera_views = [
    ("fixed1", cam_fixed1),
    ("fixed2", cam_fixed2),
...
for lbl in result.labels_all
    println("Rendering $lbl default ...")
    render_by_label(result, lbl)

    for (name, camera) in camera_views
        println("Rendering $lbl $name ...")
        render_by_label(result, lbl; cam=camera, suffix=name)
    end
end
println("All scenes saved.")
...
## OLDER CODE BELOW  ## no need to read it
