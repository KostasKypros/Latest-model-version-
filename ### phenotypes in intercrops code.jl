...
sw = max(0.1, sw_sp1 + gap + sw_sp2)
    ...
    #
    CSV.write("$(label)_n_uptake_by_depth.csv", vcat(nk_all...))
    CSV.write("$(label)_fn_components.csv",     vcat(fn_all...))
    CSV.write("$(label)_soil_N_per_voxel.csv",  vcat(sn_all...))
...
# ═══════════════════════════════════════════════════════════════════
# SECTION: EXPERIMENT MATRIX — 6 configs × 2 N levels = 12 runs
# ═══════════════════════════════════════════════════════════════════
function run_experiment_matrix(; n_days::Int = 80, start_DOY::Int = 180,
                                 sv::Float64 = 0.01)

    # Fixed field geometry — identical inputs for EVERY run (per decision 2D):
    fixed = (
        sp1 = :cereal2, sp2 = :cereal2,      # both slots are Species 2 phenotypes
        n_plants_row_sp1 = 12, n_plants_row_sp2 = 12,
        n_rows_sp1 = 1,        n_rows_sp2 = 1,
        plant_spacing_sp1 = 0.02, plant_spacing_sp2 = 0.02,
        row_spacing_sp1 = 0.2,    row_spacing_sp2 = 0.2,
        inter_row_spacing = 0.2,
    )

    # 6 phenotype pairings (Row A = pheno1, Row B = pheno2)
    configs = [
        (:reference, :reference), (:topsoil, :topsoil), (:deep, :deep),   # monocrop
        (:reference, :topsoil),   (:reference, :deep),  (:deep, :topsoil), # mixed
    ]
    N_levels = [30000.0, 15000.0]   # ample, limiting (µmol/L)

    footprints = String[]
    for (p1, p2) in configs, N in N_levels
        label = "$(p1)_$(p2)_N$(Int(N))"
        run_full_model(; n_days=n_days, start_DOY=start_DOY, sv=sv,
                        soil_N_init=N, label=label, 
                        pheno1=p1, pheno2=p2, fixed...)

        # ── Footprint invariant check from the exported position CSV ──
        pos = CSV.read("$(label)_plant_positions.csv", DataFrame)
        fx = maximum(pos.x) - minimum(pos.x)
        fy = maximum(pos.y) - minimum(pos.y)
        push!(footprints, "$label: x-span=$(round(fx,digits=4)) y-span=$(round(fy,digits=4))")
    end

    println("── Footprint check (must be identical across all 12 runs) ──")
    foreach(println, footprints)
    if length(unique(replace.(footprints, r"^[^:]+: " => ""))) != 1
        @warn "Footprint NOT constant across runs — investigate spacing/geometry."
    end
    return nothing
end


run_experiment_matrix(; n_days = 35, start_DOY = 180, sv = 0.04)
