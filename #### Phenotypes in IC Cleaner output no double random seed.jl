#### Phenotypes in IC Cleaner output no double random seed 
...
function run_full_model(; n_days::Int = 80,
                          ...
                          write_split::Bool = false,   # ← to turn off the split csv writes
...
sw = max(0.1, sw_sp1 + gap + sw_sp2)
    ...
    plant_row   = Dict(p[4] => p[3] for p in positions)   # plant_id → row
    plant_pheno = Dict(p[4] => p[5] for p in positions)   # plant_id → phenotype
    pp_all = DataFrame[]
    pv_all = DataFrame[]
    ...
    for i in 1:n_days
        ...
        pp_day, pv_day = root_voxel_occupancy(forest_local, soil_local, plant_ids, DOY;
                                              plant_row=plant_row, plant_pheno=plant_pheno)
        push!(pp_all, pp_day)
        push!(pv_all, pv_day)
        #
        ...
        for (p, tree) in enumerate(forest_local)
            ...
            pid = plant_ids[p]
            push!(fn_all, DataFrame(DOY=DOY, plant_id=pid,
                                    row=get(plant_row, pid, 0),
                                    phenotype=get(plant_pheno, pid, :unknown),
                                    fn=fn_v, CN=CN_v, CNcrit=CNcrit_v))
        end
    end

    # ── Write Feature 2 CSVs (before return, not after!) ──
       # ── Assemble per-run DataFrames (no unconditional disk writes) ──
    pp_full = vcat(pp_all...)                      # per-plant root summary
    pv_full = vcat(pv_all...)                      # per-voxel root data (split by seg_type)
    nk_full = vcat(nk_all...)                      # N uptake by depth
    fn_full = vcat(fn_all...)                      # fn / CN / CNcrit per plant per day
    sn_full = vcat(sn_all...)                      # remaining soil N per voxel per day

    # ── Merge remaining soil N INTO the per-voxel root table (left-join on cell key) ──
    # sn_full has one row per (DOY, cell); pv_full may have several seg_type rows per cell.
    if !isempty(pv_full) && !isempty(sn_full)
        sn_key = select(sn_full, [:DOY, :cell, :soil_N])
        pv_full = leftjoin(pv_full, sn_key, on = [:DOY, :cell])
    end

    # ── Optional old split writes (is off by default unless we change it ) ──
    if write_split
    ...
    # ── Build the metadata row as a DataFrame (so the driver can stack it) ──
    meta_df = write_run_metadata(
        "run_metadata_$(label).csv";           # path is ignored when write=false (see Patch 2)
        write = write_split,
        label = label, sp1 = sp1, sp2 = sp2,
        n_plants_row_sp1 = n_plants_row_sp1, plant_spacing_sp1 = plant_spacing_sp1,
        n_rows_sp1 = n_rows_sp1, row_spacing_sp1 = row_spacing_sp1,
    ...
    # ── Extended return: add positions/maps + all per-run frames for driver stacking ──
...
for (k, lbl) in enumerate(labels_all)
    diag_rep_k = mean_diag(all_diags[k])
    savefig(plot_diagnostics(diag_rep_k; title_prefix=lbl), "diag_$(lbl).png")
    savefig(plot_carbon_budget(diag_rep_k; title_prefix=lbl), "carbon_budget_$(lbl).png")
end

# ── mean summary plot ──
fig_mean = plot_diagnostics(diag_rep; title_prefix="sv=$(sv) m (global mean)")
savefig(fig_mean, "diag_sv$(sv)_mean.png")
display(fig_mean)

# ── scene renders (re-runnable without re-running model) ──
cam_eye    = GLMakie.Vec3f(0.01326, 1.5750, 0.0016)
cam_lookat = GLMakie.Vec3f(-0.0022, 0.1750, -0.2758)
cam_up     = GLMakie.Vec3f(0.0012, -0.1750, 0.9845)

for k in eachindex(forest_all)
    lbl = labels_all[k]
    try
        fig = render_forest(forest_all[k], ss_all[k], sm_all[k])

        GLMakie.save("scene_$(lbl).png", fig)
        println("Saved: scene_$(lbl).png")

        scene = fig.scene
        cam = GLMakie.cameracontrols(scene)
        GLMakie.update_cam!(scene, cam_eye, cam_lookat, cam_up)
        GLMakie.update!(scene)
        GLMakie.save("scene_$(lbl)_fixedcam.png", fig)
        println("Saved: scene_$(lbl)_fixedcam.png")
    catch e
        @warn "Render failed for $lbl" exception=e
    end
end
...
for k in eachindex(forest_all)
    ...
    # default angle
    GLMakie.update!(fig.scene)           # ensure render state is current
    GLMakie.save("scene_$(lbl).png", fig)
    println("  Saved: scene_$(lbl).png")
    ...
    cam = GLMakie.cameracontrols(scene)
    GLMakie.update_cam!(scene, cam_eye, cam_lookat, cam_up)
    GLMakie.update!(scene)                # <-- apply the camera move before capture
    GLMakie.save("scene_$(lbl)_fixedcam.png", fig)
    println("  Saved: scene_$(lbl)_fixedcam.png")
end
 ...
 # ── (6) end-of-run scene PNG loop (default + fixed camera) ──
    cam_eye    = GLMakie.Vec3f(0.01326, 1.5750, 0.0016)
    cam_lookat = GLMakie.Vec3f(-0.0022, 0.1750, -0.2758)
    cam_up     = GLMakie.Vec3f(0.0012, -0.1750, 0.9845)

    for k in 1:length(forest_all)
        lbl = labels_all[k]
        try
            fig = render_forest(forest_all[k], ss_all[k], sm_all[k])
            GLMakie.save("scene_$(lbl).png", fig)
